...
def test_tc_mb_cus_007(browser_config, test_case):
    logging.info("TC_MB_CUS_007 Started..")
    logging.info(test_case["customer"][6]["TC_CUS_007"]["_comment"])
    ...
    try:
        customer_registration_page.enter_customer_fullname(test_case["customer"][6]["TC_CUS_007"]["fullname"])
    ...
    try:
        customer_registration_page.enter_customer_email(test_case["customer"][6]["TC_CUS_007"]["email"])
    ...
    try:
        customer_registration_page.enter_customer_password(test_case["customer"][6]["TC_CUS_007"]["pass"])
    ...
    # 6. Enter invalid value (empty) of Initial Deposit Amount
    try:
        customer_registration_page.enter_customer_ini_deposit(test_case["customer"][6]["TC_CUS_007"]["deposit_amount"])
        logging.info("Invalid Initial Deposit Amount (empty) Enter successfully.")
    except Exception as e:
    ...
    expected_error_message = test_case["customer"][6]["TC_CUS_007"]["expected_result"]

    if expected_error_message == customer_registration_page.get_html_popup_error_message("InitialDeposit"):
